...
module.exports = function Conversion(config) {
    ...
    this._pipeWidth           = this._config.pipe_width !== undefined &&
                                ...
                                ? Math.abs(+this._config.pipe_width)
                                : this._maxPoolSizeTarget;

    this._pipeWidth           = this._pipeWidth > this._maxPoolSizeTarget ? this._maxPoolSizeTarget : this._pipeWidth;
    this._targetConString     = 'postgresql://' + this._targetConString.user + ':' + this._targetConString.password
                                + '@' + this._targetConString.host + ':' + this._targetConString.port + '/'
                                + this._targetConString.database + '?client_encoding=' + this._targetConString.charset;
};
