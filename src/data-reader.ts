...
const populateTable = async (conv: Conversion, chunk: Record<string, any>): Promise<void> => {
    ...
    const sqlCopy = `COPY "${conv._schema}"."${tableName}" (${copyColumnNamesList}) FROM STDIN 
        WITH (FORMAT csv, DELIMITER '${conv._delimiter}', ENCODING '${conv._targetConString.client_encoding}');`;
...
const getJson2csvStream = async (
    ...
    const options: Record<string, any> = {
        ...
        header: false,
        fields: tableColumnsResult.data.map((column: Record<string, any>): string => column.Field),
    };
