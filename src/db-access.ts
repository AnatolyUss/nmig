...
export default class DbAccess {
    ...
    public static closeConnectionPools = async (conversion: Conversion): Promise<Conversion> => {
        const closeMySqlConnections = (): Promise<void> => {
            return new Promise<void>(resolve => {
                if (conversion._mysql) {
                    conversion._mysql.end(
                        async (error: NodeJS.ErrnoException | null): Promise<void> => {
                            if (error) {
                                await generateError(
                                    conversion,
                                    `\t--[${closeMySqlConnections.name}] ${error}`,
                                );
                            }
        ...
        const closePgConnections = async (): Promise<void> => {
            if (conversion._pg) {
                try {
                    ...
                    await generateError(conversion, `\t--[${closePgConnections.name}] ${error}`);
                }
            }
        };
        ...
        await log(
            conversion,
            `\t--[${DbAccess.closeConnectionPools.name}] Closed all DB connections.`,
        );
        return conversion;
    };
    ...
    public static releaseDbClient = async (
        ...
        try {
            ...
            await generateError(conversion, `\t--[${DbAccess.releaseDbClient.name}] ${error}`);
        }
    };
    ...
    public static query = async (
        queryParams: DBAccessQueryParams,
