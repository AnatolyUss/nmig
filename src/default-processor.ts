...
import { mapDataTypes } from './table-processor';
...
export default async (conversion: Conversion, tableName: string): Promise<void> => {
    ...
    const _cb = async (column: Record<string, any>): Promise<void> => {
        const pgSqlDataType: string = mapDataTypes(
        ...
        const isOfBitType = !!pgSqlBitTypes.find((bitType: string): boolean =>
    ...
    const promises: Promise<void>[] = (
        ...
        .filter((column: Record<string, any>): boolean => column.Default !== null)
        .map(_cb);
