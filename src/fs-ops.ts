...
const readAndParseJsonFile = (pathToFile: string): Promise<any> => {
    return new Promise<any>(resolve => {
        fs.readFile(pathToFile, (error: NodeJS.ErrnoException | null, data: Buffer) => {
            ...
            const config: Record<string, any> = JSON.parse(data.toString());
            resolve(config);
        });
    });
};
...
export const readDataAndIndexTypesMap = async (conversion: Conversion): Promise<Conversion> => {
    ...
    await log(
        conversion,
        `\t--[${readDataAndIndexTypesMap.name}] Data and Index Types Maps are loaded...`,
    );
    return conversion;
};
...
export const createLogsDirectory = async (conversion: Conversion): Promise<Conversion> => {
    await createDirectory(conversion, conversion._logsDirPath, createLogsDirectory.name);
    await createDirectory(conversion, conversion._notCreatedViewsPath, createLogsDirectory.name);
    return conversion;
};
