...
import { logInBackground, generateErrorInBackground } from './fs-ops';
...
process.on('message', async (logMessage: LogMessage): Promise<void> => {
    try {
        if (logMessage.type === LogMessageType.CONFIG) {
            ...
            conv = conv || new Conversion(logMessage.config as Record<string, any>, avoidLogger);
        ...
        console.log(`\n\t--[logs-processor] Logger error: ${JSON.stringify(error)}\n`);
    }
});
