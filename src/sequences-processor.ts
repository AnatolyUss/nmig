...
export const setSequenceValue = async (
    ...
    const _isAutoIncremented = (column: Record<string, any>): boolean =>
        column.Extra === 'auto_increment';

    const autoIncrementedColumn: Record<string, any> | undefined =
        conversionTable.arrTableColumns.find(_isAutoIncremented);
    ...
    if (!result.error) {
        await log(
            conversion,
            `\t--[${setSequenceValue.name}] Sequence "${conversion._schema}"."${seqName}" value is set...`,
...
export const createIdentity = async (conversion: Conversion, tableName: string): Promise<void> => {
    ...
    const _cb = (column: Record<string, any>): boolean => column.Extra === 'auto_increment';
    const autoIncrementedColumn: Record<string, any> | undefined =
        conversionTable.arrTableColumns.find(_cb);
    ...
    if (!sqlSetSequenceValueResult.error) {
        const successMsg = `\t--[${createIdentity.name}] Added IDENTITY for ${fullTableName}."${columnName}"...`;
