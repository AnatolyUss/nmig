...
export const mapDataTypes = (
    objDataTypesMap: Record<string, any>,
...
export const createTable = async (conversion: Conversion, tableName: string): Promise<void> => {
    await log(
        conversion,
        `\t--[${createTable.name}] Currently creating table: \`${tableName}\``,
    ...
    const columnsDefinition: string = columns.data
        .map((column: Record<string, any>): string => {
            ...
            const colType: string = mapDataTypes(
                conversion._dataTypesMap as Record<string, any>,
                column.Type,
            );
    ...
    if (!createTableResult.error) {
        await log(
            conversion,
            `\t--[${createTable.name}] Table "${conversion._schema}"."${tableName}" is created...`,
            (conversion._dicTables.get(tableName) as Table).tableLogPath,
        );
    }
};
